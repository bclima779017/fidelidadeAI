...
def generate_report(results: list[dict], rag_metadata: dict | None = None, score_ponderado: float | None = None, suggestions_data: list[dict] | None = None, return_bytes: bool = False) -> "str | tuple[io.BytesIO, str]":
    """Gera o relatório .xlsx com formatação condicional.
    ...
    Args:
        ...
        return_bytes: Se True, retorna (BytesIO, filename) em vez de salvar em disco.

    Returns:
        Se return_bytes=False: caminho do arquivo salvo (str).
        Se return_bytes=True: tupla (BytesIO com conteúdo xlsx, filename).
    """
    ...
    if return_bytes:
        ...
        output_target = os.path.join(OUTPUT_DIR, filename)

    filepath = output_target
